...
const Editor = () => {
  ...
  const handleKeyDown = (event, sceneIndex, lineIndex) => {
    ...
    if (event.key === "Enter") {
      ...
      // Only after pressing Enter and Tab, add the character
      if (input.style.textAlign === "center" && currentLine) {
        setCharacters([...characters, currentLine.trim()]);
      }
        ...
        {scenes.map((scene, sceneIndex) => (
          <div key={scene.id} className="w-11/12 bg-white rounded-lg lg:w-2/3">
            <div className="w-full bg-pink-100 p-4 mb-6 rounded-md shadow-md">
              ...
              {/* Display the character names here */}
              <div className="mb-3 flex items-center gap-2">
                <span className="text-xl font-medium">Characters:</span>
                {characters.map((char, index) => (
                  <span
                    key={index}
                    className="text-xl font-bold text-white bg-gray-400 px-2 py-1 rounded-full"
                  >
                    {char}
                  </span>
                ))}
                <button className=" inline-flex items-center text-xl font-bold justify-center text-white px-1.5 border border-black bg-gray-400 
                rounded shadow hover:bg-pink-500 hover:text-white">
                  +
                </button>
              </div>
            </div>
  ...
  <input
    ...
    className={`w-[98%] text-xl bg-transparent outline-none pb-2 pl-2 ${lineIndex > 0 ? "pl-8" : ""}`}
  />
))}
